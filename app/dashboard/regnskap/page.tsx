    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">ERP-integrasjon, synk og varebestilling</p>
        </div>
      </div>
        ...
        <div className="space-y-6 xl:col-span-8">
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div className="space-y-1">
                    ...
                    {activeIntegration.last_sync_at ? (
                      <p className="text-xs text-muted-foreground">
                        ...
                        {activeIntegration.last_sync_status && (
                          <span className={`ml-1.5 ${statusColor(activeIntegration.last_sync_status)}`}>
                    ...
                    {activeIntegration.last_sync_error && (
                      <p className="max-w-md text-xs text-destructive">{activeIntegration.last_sync_error}</p>
                    )}
                  </div>

                  <form action={queueManualSync} className="shrink-0">
                    <input type="hidden" name="integration_id" value={activeIntegration.id} />
                    <SubmitButton size="sm" variant="outline">Kø manuell synk</SubmitButton>
                  </form>
                </div>
                      ...
                      {integrations
                        ...
                        .map((i) => (
                          <p key={i.id} className="text-xs text-muted-foreground">
                            {providerLabel(i.provider)} · {i.status === "inactive" ? "Inaktiv" : "Feil"}
                          </p>
                        ))}
                    </div>
                  </div>
          ...
          <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
            ...
            <p className="mb-4 text-sm text-muted-foreground">
              Velg leverandør og fyll inn tilgangsinformasjon. API-nøkler lagres kryptert i Supabase Vault.
            </p>
            <IntegrationSetupForm
              action={upsertIntegration}
              currentProvider={(activeIntegration?.provider ?? null) as "fiken" | "tripletex" | null}
          ...
          <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
            ...
            {jobs.length === 0 ? (
              ...
              <div className="divide-y divide-border/60">
                {jobs.map((job) => (
                  <div key={job.id} className="flex items-start justify-between gap-4 py-3 text-sm">
                    <div className="min-w-0 flex-1">
                      <p className="truncate font-medium">
                        {job.orders?.title ?? (job.order_id ? `Ordre ${job.order_id.slice(0, 8)}…` : "Generell synk")}
                      </p>
                      {job.last_error && <p className="mt-0.5 truncate text-xs text-destructive">{job.last_error}</p>}
                      {job.next_retry_at && job.status === "retry_wait" && (
                        <p className="mt-0.5 text-xs text-muted-foreground">Neste forsøk: {formatDate(job.next_retry_at)}</p>
                      )}
                    </div>
                    <div className="shrink-0 text-right">
                      <p className={`text-xs font-medium ${statusColor(job.status)}`}>{statusLabel(job.status)}</p>
                      <p className="mt-0.5 text-xs text-muted-foreground">{formatDate(job.created_at)}</p>
                      {job.retry_count > 0 && (
                        <p className="mt-0.5 text-xs text-muted-foreground">{job.retry_count}/{job.max_retries} forsøk</p>
                      )}
                    </div>
                  </div>
                ))}
      ...
      <div className="grid gap-6 xl:grid-cols-12">
        <div className="rounded-xl border border-border bg-card p-6 shadow-sm xl:col-span-7">
          <h2 className="mb-1 text-base font-semibold">Varebestilling</h2>
          <p className="mb-4 text-sm text-muted-foreground">
            Bestill varer fra lokal katalog. Ekstern leverandørkatalog kan kobles på samme flyt senere.
          </p>
          <form action={createPurchaseOrder} className="grid gap-3 md:grid-cols-6">
            ...
            <div className="space-y-1 md:col-span-2">
              <label htmlFor="supplier_note" className="text-xs font-medium text-muted-foreground">Leverandørnotat</label>
              <input
                id="supplier_note"
                name="supplier_note"
                placeholder="konto / filial / ref"
                className="h-10 w-full rounded-lg border border-input bg-background px-3 text-sm shadow-sm outline-none focus-visible:ring-2 focus-visible:ring-ring"
              />
            </div>
            ...
            <div className="flex items-end md:col-span-1">
              <SubmitButton className="w-full">Opprett bestilling</SubmitButton>
            </div>
          </form>
        </div>

        <div className="rounded-xl border border-border bg-card p-6 shadow-sm xl:col-span-5">
              ...
              {purchaseOrders.map((order) => (
                <div key={order.id} className="flex flex-col gap-2 py-3 sm:flex-row sm:items-center sm:justify-between">
                  <div className="min-w-0">
                    <p className="text-sm font-medium">{order.supplier_name} · {purchaseStatusLabel(order.status)}</p>
                    <p className="truncate text-xs text-muted-foreground">
                      {(order.purchase_order_lines ?? [])
                        .map((line) => `${line.item_name} (${line.quantity} ${line.unit})`)
                        .join(", ") || "Ingen linjer"}
                    </p>
                    <p className="text-xs text-muted-foreground">
